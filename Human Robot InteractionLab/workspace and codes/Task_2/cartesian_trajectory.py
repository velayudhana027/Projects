...
class Node(object):
    ...
    def __init__(self):
        ...
        self.initial_pos = np.array([[1,0,0,0.3069],[0,-1,0,0],[0,0,-1,0.4873],[0,0,0,1]])
    ...
    def run(self):
        ...
        for k in range(N_steps):
            ...
            """
            Inverse kinematics for each cartesian point of the trajectory.
            Initialization of the inverse kinematics function of the rtb-toolbox
            with the previous joint configuration.
            Numerical inverse kinematics by Levenberg-Marquadt optimization.
            TODO Inverse kinematics for each point, store to joint space trajectory
            """
            if k == 0:
                traj_joint_space_k = self.panda.ikine_LM(SE3(...), q0=self.start_q)
            else:
                traj_joint_space_k = self.panda.ikine_LM(SE3(...), q0=traj_joint_space[k-1])
        ...
        while not rospy.is_shutdown():
            ...
            """
            if i < N_steps: 
                self.joint_pos_msg.points[0].positions = ...
            else:
                self.joint_pos_msg.points[0].positions = traj_joint_space[N_steps-1]
...
if __name__ == '__main__':
    rospy.init_node('cartesian_trajectory', anonymous=False)
    n = Node()
    n.run()
